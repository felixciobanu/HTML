...
public class CartModel : PageModel
{
	...
	public async Task OnPostAsync(string customerName, Location location)
	{
		var cartId = HttpContext.Session.GetGuid("CartId");
		if (cartId is null) return;
		var res = await _mediator.Send(new CartCheckout.Request(cartId.Value, customerName, location));
		HttpContext.Session.SetGuid("CartId", "");
	}
}
}