...
def select_grades_given_student(conn, student_no):
    cur = conn.cursor()
    cur.execute = ("SELECT student_no, course_id, grade FROM grades WHERE student_no={}".format(student_no,))
    grades = []
    for (nr, id, grade) in cur:
        grades.append({
            "student_no": nr,
...
def select_students_given(conn, student_no):
    cur = conn.cursor()
    cur.execute = ("SELECT student_no, name "
                   "FROM students WHERE student_no=?", student_no)
    print(cur.fetchall())
    students = []
    for (student_no, name) in cur:
        students.append({
